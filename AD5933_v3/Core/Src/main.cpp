...
const char *initTest[5] = { "Reset", "StatFreq", "StepFreq", "StepCount",
		"OutputRange" };
...
bool freqSweep(UartDMA &uart1, AD5933 &ad5933, int16_t real[], int16_t imag[],
		settings set);
...
uint8_t parseData(char *str, settings *set) {
	...
	const char *commands[] = { "StartSweep", "RepeatSweep", "Calibrate",
			"GetData" };
	const char *modifiers[] = { "StartFrequency", "StepFrequency", "StepCount",
			"RepeatCount" };
	if (token != NULL) {
		for (int i = 0; i < 4; i++) {
