...
bool
freqSweep (UartDMA &uart1, AD5933 &ad5933, int16_t real[], int16_t imag[],
	   settings set);
uint8_t
parseData (char *str, settings *set);
bool
setParameters (AD5933 &ad5933, settings set, int16_t *real, int16_t *imag);
...
main (void)
{
  ...
  uart1.start_read ();
  Dout pin1 (GPIOA, GPIO_PIN_6, GPIO_PIN_RESET);
  Dout pin2 (GPIOA, GPIO_PIN_7, GPIO_PIN_RESET);
  Dout pin3 (GPIOB, GPIO_PIN_6, GPIO_PIN_RESET);
//  HAL_Delay(1000);
...
//  uint8_t repeatCount = 1;
  ...
  ad5933.calibrate (gain, 8200, 10, 5);
  int16_t *real;
  int16_t *imag;
  while (1)
    {
      ...
      if (newData)
	{
	  switch (parseData (str, &set))
	    ...
	    case 3:
	      ...
	      ad5933.calibrate (gain, 820, set.IncrementCount, set.RepeatCount);
	   ...
	   settings set)
{
  ...
  while ((ad5933.readRegister (STATUS_REG) & STATUS_SWEEP_DONE)
      ...
      else
	{
	  ...
	  real[i] = (int16_t) sumRe / set.Averaging;
	  imag[i] = (int16_t) sumRe / set.Averaging;
	  sumRe = 0;
	  sumIm = 0;
