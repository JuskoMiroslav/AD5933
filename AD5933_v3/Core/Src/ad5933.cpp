...
AD5933::AD5933(I2C_HandleTypeDef *hi2c) :
		_hi2c(hi2c) {
}

HAL_StatusTypeDef AD5933::readByte(I2C_HandleTypeDef *hi2c, uint8_t reg,
		uint8_t *data) {
	uint8_t adr[1] = { 0xB0 };
...
uint8_t AD5933::init(uint32_t StartFreq, uint32_t IncrementFreq,
		uint32_t IncrementCount, HAL_OutputRange OutRange, bool ExternalClock =
				true) {
	if (reset() != HAL_OK)
		return 1;
...
uint8_t AD5933::init(uint32_t StartFreq, uint32_t IncrementFreq,
		uint32_t IncrementCount, HAL_OutputRange OutRange) {
	return init(StartFreq, IncrementFreq, IncrementCount, OutRange, true);
}

HAL_StatusTypeDef AD5933::writeByte(I2C_HandleTypeDef *hi2c, uint8_t reg,
		uint8_t data) {
	return HAL_I2C_Mem_Write(hi2c, AD5933_ADDR, reg, I2C_MEMADD_SIZE_8BIT,
			&data, 1, 1000);
}
...
HAL_StatusTypeDef AD5933::calibrate(double gain[], int ref, int n,
	...
	if (frequencySweep(real, imag, n, repeatCount) != HAL_OK) {
		free(real);
		free(imag);
		return HAL_ERROR;
	}
